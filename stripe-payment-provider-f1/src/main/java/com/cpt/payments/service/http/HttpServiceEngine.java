...
public class HttpServiceEngine {
	...
	public HttpServiceEngine(RestTemplate restTemplate) {
		...
		System.out.println("HttpServiceEngine.HttpServiceEngine() | "
	...
	public ResponseEntity<String> makeHttpRequest(HttpRequest httpRequest) {
		try {
			...
			System.out.println("Got clent/server error while making Http call : " + ex);
			ex.printStackTrace();
			return ResponseEntity.status(ex.getStatusCode()).body(ex.getResponseBodyAsString());
			
		}catch(ResourceAccessException ex) {
			System.out.println("Got timeout while making http call : " + ex);
			ex.printStackTrace();
